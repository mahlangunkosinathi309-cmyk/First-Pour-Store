...
# IMPORTANT: set this in Render as FLASK_SECRET_KEY (or SECRET_KEY)
app.secret_key = os.getenv("FLASK_SECRET_KEY") or os.getenv("SECRET_KEY") or "dev-secret-change-me"

WHATSAPP_NUMBER = os.getenv("WHATSAPP_NUMBER", "27645277314")  # default example
...
def checkout():
    ...
    if request.method == "POST":
        ...
        # Save customer details for WhatsApp message + optional payment metadata
    ...
    return render_template(
        ...
        whatsapp_number=WHATSAPP_NUMBER,
        lines=lines,
        subtotal_display=cents_to_zar(total),
...
def yoco_start():
    ...
    # Create a local order id (clientReferenceId)
    ...
    headers = {
        ...
        # Optional idempotency key (prevents duplicates if user double-clicks)
        "Idempotency-Key": order_id,
    }

    r = requests.post(YOCO_CHECKOUT_URL, json=payload, headers=headers, timeout=30)
    if r.status_code != 200:
        # If Yoco rejects, go back to checkout and show fallback
        session["yoco_error"] = f"Yoco error: {r.status_code}"
        return redirect(url_for("checkout"))

    data = r.json()
    session["yoco_checkout_id"] = data.get("id", "")
...
def payment_success():
    # NOTE: Yoco warns return URLs are NOT proof of payment.
    # For MVP, we show success and send WhatsApp confirmation message.
